...
static void execute_command() {
  ...
  if (kstrcmp(buffer, "help") == 0) {
    vga_puts("Files: ls, mkdir <name>, mkfile <name> [content], rm <name>, cat "
             "<name>, cd <name>\n");
    ...
    asm volatile("outw %1, %0"
                 :
                 : "dN"((uint16_t)0x604), "a"((uint16_t)0x2000)); // QEMU
    asm volatile("outw %1, %0"
                 :
                 : "dN"((uint16_t)0xB004),
                   "a"((uint16_t)0x2000)); // Bochs/Older QEMU
    while (1)
      asm volatile("hlt");
  } else if (kstrcmp(buffer, "layout") == 0 ||
             (buffer[0] == 'l' && buffer[1] == 'a' && buffer[2] == 'y' &&
              buffer[3] == 'o' && buffer[4] == 'u' && buffer[5] == 't' &&
              buffer[6] == ' ')) {
    if (buffer[6] == 0) {
  ...
  } else if (kstrcmp(buffer, "pkg") == 0 ||
             (buffer[0] == 'p' && buffer[1] == 'k' && buffer[2] == 'g' &&
              (buffer[3] == ' ' || buffer[3] == 0))) {
    if (buffer[3] == 0) {
      ...
      if (kstrcmp(app, "matrix") == 0 || kstrcmp(app, "calc") == 0 ||
        ...
        vga_puts(app);
        vga_puts(" installed successfully. Type '");
        vga_puts(app);
        vga_puts("' to run.\n");
      } else {
    ...
    vga_puts("Current resolution: 80x25 (Standard VGA Text Mode)\n");
             ...
             buffer[3] == 'i' && buffer[4] == 'r' && buffer[5] == ' ') {
    vfs_mkdir(buffer + 6);
  } else if (buffer[0] == 'm' && buffer[1] == 'k' && buffer[2] == 'f' &&
             buffer[3] == 'i' && buffer[4] == 'l' && buffer[5] == 'e' &&
             buffer[6] == ' ') {
    ...
    if (space_pos != -1) {
      ...
      if (vfs_find_bin_app(cmd_name)) {
        if (kstrcmp(cmd_name, "calc") == 0)
          app_calc(buffer + space_pos + 1);
        else if (kstrcmp(cmd_name, "edit") == 0)
          app_edit(buffer + space_pos + 1);
        goto prompt_no_nl;
      }
    }
